...
def hawkins(T, R, C, B, start_state, lambda_lim=None, gamma=0.95):
	...
	m.setObjectiveN(sum([L[i].dot(mu[i]) for i in range(NPROCS)]) + lam*B*((1-gamma)**-1), 0, 1)
...
def hawkins_fixed_bt(H, T, P, R, C, B, bts, bvals, start_state, lambda_lim=None, gamma=0.95):
