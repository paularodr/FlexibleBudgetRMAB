...
def get_hawkins_actions(H, N, T, R, C, B, current_state, gamma):
    actions = np.zeros(N)
    ...
    L_vals, lambda_val = hawkins_methods.hawkins_finite(H,T, R, C, B, current_state, gamma=gamma)

    for i in range(N):
        for a in range(T.shape[2]):
            for s in range(T.shape[1]):
                Q_vals[i,a,s] = R[i,s] - C[a]*lambda_val + gamma*L_vals[i,:,0].dot(T[i,s,a])
